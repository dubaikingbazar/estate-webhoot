...
Tum "${brokerName}" ke real estate consultant ho. Natural, warm Hinglish mein baat karo. Kabhi AI/bot mat batao.
...
Step 4 — Property details (INTENT KE HISAAB SE):

AGAR KHARIDNA ya RENT LENA:
  Flat: BHK, furnished, parking, lift/society, family size, loan ya cash
  House/Makan: kamre, plot size (gaj), manzil, ground floor
  Kothi: plot size, kamre, garden, loan ya cash
  Villa: plot size, floors, garden/parking
  Plot: size (gaj/acre), corner ya normal, registry
  Dukan/Shop: area (sqft), main road ya andar, parking
  Office: carpet area, kitne log, parking
  Industrial: warehouse/factory/shed, area, highway, power supply
  Farm House: acre, construction chahiye

AGAR SELL ya RENT DENA:
  Flat: BHK, furnished, area, age, negotiable, registry
  House/Makan: kamre, plot size, manzil, age, registry, negotiable
  Kothi: plot size, kamre, age, registry
  Plot: size, corner, registry, jaldi bechna
  Dukan/Shop: area, main road, age, negotiable
  Industrial: type, area, highway distance
  Farm House: acre, construction, registry

  SELL/RENT DENA mein KABHI MAT PUCHHO: lift, parking chahiye, society

Step 5 — Location:
"Kaunsa area prefer karenge?"

Step 6 — SELL/RENT DENA ke liye PHOTOS (budget se PEHLE):
Agar SELL ya RENT DENA hai — EXACTLY YE LIKHO:
"[Naam] ji, apni property ki photos yahan upload karein — jaldi buyer milega: |||PHOTO_LINK|||"
Customer response aane do, phir budget puchho.

Step 7 — Budget:
"Budget range roughly kitni hai?"

Step 8 — Timeline:
"Kitne time mein lena/dena chahenge?"

Step 9 — Phone (KABHI SKIP MAT KARO):
"[Naam] ji, apna WhatsApp number do — hamare advisor call karenge. Free consultation hai!"

Jab naam, phone, property type, area, budget sab mile — EXACTLY YE LIKHO:
|||LEAD|||{"name":"NAAM","phone":"PHONE","type":"TYPE","area":"AREA","budget":"BUDGET","intent":"Rent Lena Chahte Hain/Rent Dena Chahte Hain/Kharidna Chahte Hain/Sell Karna Chahte Hain","timeline":"TIMELINE","furnished":"Furnished/Semi-Furnished/Unfurnished/NA","parking":"Chahiye/Nahi Chahiye/NA","special":"REQUIREMENTS"}|||

Thank you message:
Agar SELL ya RENT DENA — EXACTLY:
"Bahut shukriya [Naam] ji! Details note ho gayi hain. Photos yahan upload karein — jaldi buyer milega: |||UPLOAD_LINK||| Senior advisor kal tak call karenge!"

Agar KHARIDNA ya RENT LENA — EXACTLY:
"Bahut shukriya [Naam] ji! Saari details note ho gayi hain. Senior advisor kal tak call karenge. Koi sawaal ho toh pooch sakte ho!"

STRICT: |||UPLOAD_LINK||| aur |||PHOTO_LINK||| exact waise hi likho.
STRICT: Ek sawaal ek baar. Phone bina lead complete mat karo.
`;
...
<table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;width:100%;">
  <tr><td style="background:linear-gradient(135deg,#1e3a5f,#2d5a8e);border-radius:14px;padding:32px 24px;text-align:center;" align="center">
    ...
    <h2 style="color:#ffffff;margin:0 0 6px;font-size:28px;font-weight:700;">${leadData.name}</h2>
    <p style="margin:0 0 16px;color:#94a3b8;font-size:13px;">${leadData.intent || 'Property Enquiry'}</p>
    <a href="tel:${leadData.phone}" style="background:#f59e0b;color:#000000;padding:13px 32px;border-radius:8px;text-decoration:none;font-weight:700;font-size:16px;display:inline-block;">&#128222; ${leadData.phone}</a>
  </td></tr>
  ...
  <tr><td style="background:#ffffff;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden;">
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr><td style="padding:14px 20px;background:#f8fafc;border-bottom:1px solid #e2e8f0;">
        <p style="margin:0;font-size:11px;font-weight:700;color:#1e3a5f;letter-spacing:2px;text-transform:uppercase;">Property Requirements</p>
      </td></tr>
      <tr><td style="padding:0;">
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr><td style="padding:14px 20px;width:44%;color:#64748b;font-size:13px;border-bottom:1px solid #f1f5f9;">Property Type</td>
            <td style="padding:14px 20px;font-weight:700;color:#1e293b;font-size:14px;border-bottom:1px solid #f1f5f9;">${leadData.type || '&mdash;'}</td></tr>
          <tr style="background:#f8fafc;"><td style="padding:14px 20px;color:#64748b;font-size:13px;border-bottom:1px solid #f1f5f9;">Preferred Area</td>
            <td style="padding:14px 20px;font-weight:700;color:#1e293b;font-size:14px;border-bottom:1px solid #f1f5f9;">${leadData.area || '&mdash;'}</td></tr>
          <tr><td style="padding:14px 20px;color:#64748b;font-size:13px;border-bottom:1px solid #f1f5f9;">Budget</td>
            <td style="padding:14px 20px;font-weight:700;color:#16a34a;font-size:18px;border-bottom:1px solid #f1f5f9;">${leadData.budget || '&mdash;'}</td></tr>
          <tr style="background:#f8fafc;"><td style="padding:14px 20px;color:#64748b;font-size:13px;">Timeline</td>
            <td style="padding:14px 20px;font-weight:700;color:#1e293b;font-size:14px;">${leadData.timeline || 'Not specified'}</td></tr>
        </table>
      </td></tr>
    </table>
  </td></tr>
  ...
  <tr><td style="background:#ffffff;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden;">
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr><td style="padding:14px 20px;background:#f8fafc;border-bottom:1px solid #e2e8f0;">
        <p style="margin:0;font-size:11px;font-weight:700;color:#1e3a5f;letter-spacing:2px;text-transform:uppercase;">Contact Details</p>
      </td></tr>
      <tr><td>
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr><td style="padding:14px 20px;width:44%;color:#64748b;font-size:13px;border-bottom:1px solid #f1f5f9;">Name</td>
            <td style="padding:14px 20px;font-weight:700;color:#1e293b;font-size:15px;border-bottom:1px solid #f1f5f9;">${leadData.name}</td></tr>
          <tr><td style="padding:14px 20px;color:#64748b;font-size:13px;">Phone / WhatsApp</td>
            <td style="padding:14px 20px;font-weight:700;color:#1e293b;font-size:22px;">${leadData.phone}</td></tr>
        </table>
      </td></tr>
    </table>
  </td></tr>
  <tr><td height="16"></td></tr>
  <tr><td align="center">
    <a href="tel:${leadData.phone}" style="background:linear-gradient(135deg,#1e3a5f,#2d5a8e);color:#ffffff;padding:16px 48px;border-radius:10px;text-decoration:none;font-weight:700;font-size:16px;display:inline-block;">&#128222; Call Now &mdash; ${leadData.phone}</a>
  </td></tr>
  <tr><td height="16"></td></tr>
  <tr><td align="center" style="color:#94a3b8;font-size:11px;">
...
<div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;background:#f8fafc;padding:20px;border-radius:16px;">
  ...
  <div style="background:#fff;padding:20px;border-radius:12px;border:1px solid #e2e8f0;">
    ...
    <p style="color:#475569;margin-top:12px;">Aapka EstateBot live ho gaya hai.</p>
...
app.post('/api/signup', async (req, res) => {
  ...
  if (!name || !business || !city || !email || !phone) return res.status(400).json({ error: 'Saari details bharo' });

  const broker_id = business.toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, '-').substring(0, 30);
  const { data: existing } = await supabase.from('brokers').select('broker_id').eq('broker_id', broker_id).single();
  const finalId = existing ? `${broker_id}-${Date.now().toString().slice(-4)}` : broker_id;

  const { error } = await supabase.from('brokers').insert([{
    broker_id: finalId, name: business, email, phone,
    city: city + (state ? ', ' + state : ''),
    specialty: specialty || 'Residential', status: 'trial',
    stats: { experience: experience ? experience + ' Yrs' : '5 Yrs', properties: properties || '100+' }
  }]);

  if (error) { console.error('Supabase error:', error); return res.status(500).json({ error: 'Database error' }); }
...
app.post('/api/temp-upload-token', (req, res) => {
  ...
  const now = Date.now();
  Object.keys(tempUploads).forEach(k => { if (now - tempUploads[k].createdAt > 7200000) delete tempUploads[k]; });
  res.json({ success: true, token, uploadUrl: 'https://estatebotai.in/upload/' + token });
});

app.get('/upload/:token', (req, res) => res.sendFile(__dirname + '/upload.html'));

app.post('/api/upload/:token', async (req, res) => {
  const { token } = req.params;
  if (token.startsWith('tmp_') && tempUploads[token]) return res.json({ success: true, temp: true, token });
  const { data: lead, error } = await supabase.from('leads').select('*').eq('upload_token', token).single();
...
app.post('/api/upload-image/:token', async (req, res) => {
  ...
  if (token.startsWith('tmp_') && tempUploads[token]) {
    ...
    const { data: lead } = await supabase.from('leads').select('id').eq('upload_token', token).single();
    if (!lead) return res.status(404).json({ error: 'Invalid token' });
    folderPath = lead.id;
  }
  const filePath = folderPath + '/' + Date.now() + '-' + fileName;
  const { data, error } = await supabase.storage.from('property-images').createSignedUploadUrl(filePath);
  if (error) return res.status(500).json({ error: error.message });
  if (token.startsWith('tmp_') && tempUploads[token]) tempUploads[token].photos.push(filePath);
...
app.post('/api/chat/:brokerId', async (req, res) => {
  ...
  try {
    ...
    if (!data.choices || !data.choices[0]) console.error('Groq error:', JSON.stringify(data));
      ...
      if (match) {
        try {
          ...
          if (!leadData.phone || leadData.phone.length < 8) {
            ...
            if (sessionToken && tempUploads[sessionToken]) delete tempUploads[sessionToken];
...
.hero-bg{position:absolute;inset:0;background:#1a1a2e;}
...
.powered{width:100%;background:#F7F4ED;border-top:1px solid #EDE7D8;padding:13px;display:flex;align-items:center;justify-content:center;gap:6px;font-size:10px;color:#6B6354;font-weight:700;}
...
<div class="hero">
  ...
  <div class="hero-content">
    <div class="broker-big-name">${broker.name.split(' ')[0]}<br><span>${broker.name.split(' ').slice(1).join(' ') || ''}</span></div>
...
<div class="chat-wrap">
  <div class="chat-wrap-inner">
    <svg class="skyline-wm" viewBox="0 0 400 140" preserveAspectRatio="xMidYMax slice">
      <rect x="0" y="95" width="22" height="45" fill="#C9A35E"/><rect x="82" y="38" width="30" height="102" fill="#C9A35E"/>
      <rect x="180" y="50" width="22" height="90" fill="#C9A35E"/><rect x="238" y="44" width="26" height="96" fill="#C9A35E"/>
...
<div class="powered">
  <svg width="20" height="20" viewBox="0 0 24 24"><circle cx="12" cy="12" r="12" fill="#0E1B30"/><path d="M7 12.5L12 8L17 12.5V17C17 17.5523 16.5523 18 16 18H8C7.44772 18 7 17.5523 7 17V12.5Z" fill="#FFFFFF"/><rect x="10.7" y="13.5" width="2.6" height="2.6" rx="0.5" fill="#D4A24C"/></svg>
...
<div class="upload-modal-overlay" id="uploadModal">
  <div class="upload-modal">
    <div class="upload-modal-header">
      ...
      <button class="upload-modal-close" onclick="closeUploadModal()">x</button>
    </div>
...
function typeMsg(text,role){
  var body=document.getElementById('chatBody');
  var d=document.createElement('div');d.className='msg '+role;
  var bubble=document.createElement('div');bubble.className='bubble';bubble.innerHTML='';
  var ts=document.createElement('div');ts.className='ts';ts.textContent=getTime();
...
function handleModalFiles(files){
  var newFiles=Array.from(files).slice(0,5-modalFiles.length);
...
function renderModalPreviews(){
  ...
  modalFiles.forEach(function(file,i){
    ...
    reader.onload=function(e){
      ...
      item.innerHTML='<img src="'+e.target.result+'"/><button class="del" onclick="removeModalFile('+i+')">x</button>';
...
async function submitModalUpload(){
  ...
  try{
    for(var i=0;i<modalFiles.length;i++){
      var file=modalFiles[i];
      var signRes=await fetch('/api/upload-image/'+currentUploadToken,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fileName:file.name,fileType:file.type})});
...
function quickSelect(btn,text){
  ...
  fetch('/api/chat/'+brokerId,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,sessionId:sessionId})})
...
async function sendMsg(){
  ...
  if(leadDone){
    ...
    if(c.includes('galat')||c.includes('wrong')||c.includes('change')||c.includes('sahi')||c.includes('galti')){
      ...
      var sb=document.querySelector('.send-btn');if(sb)sb.style.display='flex';
    }else return;
  }
  input.value='';addMsg(msg,'user');showTyping();
  try{
    var res=await fetch('/api/chat/'+brokerId,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg,sessionId:sessionId})});
