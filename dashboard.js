  ...
  document.getElementById('detailBody').innerHTML=`
    ...
    <div class="detail-top">
      ...
      <div style="flex:1;min-width:0;">
        ...
        <div class="detail-phone">${lead.phone||'—'}</div>
      </div>
      <div class="score-wrap">
        <div class="score-num" style="color:${scoreColor};">${score}%</div>
        <div class="score-lbl">Score</div>
        <div class="score-bar" style="width:50px;margin-top:4px;"><div class="score-bar-fill" style="width:${score}%;background:${scoreColor};"></div></div>
      </div>
    </div>
    ...
    <div class="action-btns">
      ...
      <button class="btn-gold" onclick="openAddFollowUp('${lead.id}','${lead.name||''}')">
        ⏰ Set Follow Up Reminder
      </button>
      <button class="btn-secondary" style="color:#EF4444;border-color:rgba(239,68,68,0.3);" onclick="if(confirm('Is lead ko archive karo?'))archiveLead('${lead.id}')">
        🗄 Archive Lead
      </button>
    </div>
    ...
    <div class="detail-section" style="margin-top:14px;">
      ...
      <div class="wa-templates">
        ${templates.map((t,i)=>`<div class="wa-tpl" onclick="sendWhatsApp('${phone}','${t.replace(/'/g,"\\'").replace(/\n/g,' ')}')">${t.substring(0,80)}${t.length>80?'...':''}</div>`).join('')}
      </div>
    </div>
  `;
...
function makeCall(phone,leadId,leadName){
  ...
  setTimeout(()=>{
    ...
    if(!callLogs[leadId])callLogs[leadId]=[];
    callLogs[leadId].push(`📞 Called ${now}`);
    localStorage.setItem('eb_calllogs',JSON.stringify(callLogs));
...
function saveNote(leadId){
  const note=document.getElementById('noteArea_'+leadId)?.value||'';
  notes[leadId]=note;
  localStorage.setItem('eb_notes',JSON.stringify(notes));
  showToast('Note saved!');
}

function logCall(leadId,leadName){
  const now=new Date().toLocaleString('en-IN',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'});
  const result=prompt('Call result likho (e.g. "Callback maanga", "Not interested", "Site visit fix hua")');
  if(result===null)return;
  if(!callLogs[leadId])callLogs[leadId]=[];
  callLogs[leadId].push(`📞 ${now} — ${result||'Called'}`);
  localStorage.setItem('eb_calllogs',JSON.stringify(callLogs));
  const el=document.getElementById('callLogDisplay_'+leadId);
  if(el)el.innerHTML=callLogs[leadId].slice(-3).map(l=>`<div style="font-size:11px;color:var(--muted);padding:4px 0;border-bottom:1px solid #F1F5F9;">${l}</div>`).join('');
  showToast('Call logged!');
}
...
async function updateStatus(leadId,newStatus){
  ...
  if(ok){
    allLeads=allLeads.map(l=>l.id===leadId?{...l,status:newStatus}:l);
    filteredLeads=filteredLeads.map(l=>l.id===leadId?{...l,status:newStatus}:l);
    filteredLeads2=filteredLeads2.map(l=>l.id===leadId?{...l,status:newStatus}:l);
    updateStats();renderTable();
    const labels={new:'New',in_progress:'In Progress',converted:'Converted ✅',not_interested:'Not Interested',follow_up:'Follow Up'};
    showToast('Status: '+(labels[newStatus]||newStatus));
...
function renderAnalytics(){
  ...
  document.getElementById('analyticsStats').innerHTML=`
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;">
      <div style="font-size:20px;font-weight:800;color:var(--navy);">${allLeads.length}</div><div style="font-size:11px;color:var(--muted);margin-top:3px;">Total Leads</div></div>
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;">
      <div style="font-size:20px;font-weight:800;color:#10B981;">${converted}</div><div style="font-size:11px;color:var(--muted);margin-top:3px;">Converted</div></div>
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;">
      <div style="font-size:20px;font-weight:800;color:var(--gold);">${convRate}%</div><div style="font-size:11px;color:var(--muted);margin-top:3px;">Conversion Rate</div></div>
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;">
      <div style="font-size:20px;font-weight:800;color:#7C3AED;">${todayLeads.length}</div><div style="font-size:11px;color:var(--muted);margin-top:3px;">Today's Leads</div></div>`;
  // Chart
  const days=[],counts=[];
...
function renderFollowUps(){
  ...
  if(!upcoming.length&&!done.length){
    document.getElementById('followUpsContainer').innerHTML=`<div style="text-align:center;padding:48px;color:var(--muted);"><div style="font-size:32px;margin-bottom:12px;">⏰</div><div style="font-size:15px;font-weight:600;color:var(--text);margin-bottom:6px;">Koi follow up nahi hai</div><div style="font-size:13px;">Leads pe reminders set karo — "Add Reminder" button se</div></div>`;
    return;
  }
  let html='';
  if(upcoming.length){
    html+=`<div style="font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">Upcoming (${upcoming.length})</div>`;
    html+=upcoming.map(f=>{
      const d=new Date(f.date+'T00:00:00');
      const isOverdue=f.date<todayStr,isToday=f.date===todayStr;
      return `<div class="fu-card" style="${isOverdue?'border-color:#EF4444;':isToday?'border-color:var(--gold);':''}">
        <div class="fu-date" style="background:${isOverdue?'#EF4444':isToday?'var(--gold)':'var(--navy)'};">
          <div class="day">${d.getDate()}</div><div>${d.toLocaleString('en-IN',{month:'short'})}</div>
        </div>
        <div style="flex:1;min-width:0;">
          <div class="fu-name">${f.leadName}</div>
          <div class="fu-note">${f.note||'No note'}</div>
          ${isOverdue?'<div style="font-size:10px;color:#EF4444;font-weight:700;">OVERDUE</div>':isToday?'<div style="font-size:10px;color:var(--gold);font-weight:700;">TODAY</div>':''}
        </div>
        <button class="fu-done-btn" onclick="markFollowUpDone('${f.id}')">✓ Done</button>
      </div>`;
    }).join('');
  }
  if(done.length){
    html+=`<div style="font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:1px;margin:14px 0 10px;">Completed (${done.length})</div>`;
    html+=done.slice(0,5).map(f=>`<div class="fu-card" style="opacity:0.5;">
      <div class="fu-date"><div class="day">${new Date(f.date+'T00:00:00').getDate()}</div><div>${new Date(f.date+'T00:00:00').toLocaleString('en-IN',{month:'short'})}</div></div>
      <div><div class="fu-name" style="text-decoration:line-through;">${f.leadName}</div><div class="fu-note">${f.note||''}</div></div>
      <div style="margin-left:auto;font-size:11px;color:var(--green);font-weight:600;">✓ Done</div>
    </div>`).join('');
  }
...
function markFollowUpDone(id){
  ...
  localStorage.setItem('eb_followups',JSON.stringify(followUps));
  renderFollowUps();showToast('Follow up complete! ✅');
}

function checkFollowUpNotifications(){
  const brokerId=currentBroker?.broker_id;
  const today=new Date().toISOString().split('T')[0];
  const todayFUs=followUps.filter(f=>f.brokerId===brokerId&&!f.done&&f.date<=today);
  if(todayFUs.length>0){
    const badge=document.getElementById('navFollowCount');
    badge.style.display='';badge.textContent=todayFUs.length;
  }
}
...
function renderSettings(){
  ...
  document.getElementById('settingsAccountInfo').innerHTML=`<div style="display:flex;flex-direction:column;gap:8px;">
    ${[['Name',currentBroker.name],['Email',currentBroker.email||'—'],['Phone',currentBroker.phone||'—'],['City',currentBroker.city||'—'],['Plan',currentBroker.status?.toUpperCase()]].map(([k,v])=>`
...
function requestNotifPermission(){
  Notification.requestPermission().then(p=>{
    if(p==='granted'){showToast('Notifications enabled! ✅');document.getElementById('notifBtn').textContent='✓ Enabled';setupLeadPolling(currentBroker.broker_id);}
...
function setupLeadPolling(brokerId){
  ...
  setInterval(async()=>{
    ...
    if(data&&data[0]&&allLeads.length>0){
      ...
      if(!found){
        ...
        if(Notification.permission==='granted'){
          new Notification('🔥 Naya Lead!',{body:`${lead.name||'New lead'} — ${lead.phone||''}`,icon:'/favicon.ico'});
        }
        showToast('🔥 Naya lead aaya — '+( lead.name||'Unknown'));
        await loadLeads(brokerId);
      }
    }
...
function getScoreExplain(lead){
  const reasons=[];
  if(lead.phone) reasons.push('✅ Phone number mila');
  else reasons.push('❌ Phone number missing');
  if(lead.budget) reasons.push('✅ Budget bataya: '+lead.budget);
  else reasons.push('❌ Budget nahi bataya');
  if(lead.area) reasons.push('✅ Location bataya: '+lead.area);
  else reasons.push('❌ Location nahi bataya');
  if(lead.timeline){
    const t=lead.timeline.toLowerCase();
    if(t.includes('urgent')||t.includes('15')||t.includes('1 month')) reasons.push('✅ Timeline urgent hai');
    else reasons.push('⚠️ Timeline: '+lead.timeline);
  } else reasons.push('❌ Timeline nahi bataya');
...
function switchLeadsTab(tab) {
  ...
  if(tab==='active') renderLeadsPage();
  else renderArchivedLeads();
}

function renderArchivedLeads() {
  const brokerId = currentBroker?.broker_id;
  // Get archived leads from leadStore
  ...
  if(!archived.length) {
    body.innerHTML = '<div class="empty-state"><h3>Koi archived lead nahi hai</h3><p>Lead detail mein jaake "Archive" button dabao</p></div>';
  ...
  body.innerHTML = archived.map(lead => {
    ...
    const av = getInitials(lead.name), avColor = getAvColor(lead.name);
    const intentShort = (lead.intent||'').replace(' Chahte Hain','').replace(' Chahte Hai','');
    return `<div class="lead-row" style="opacity:0.6;">
      <div class="lead-col-badge"><span class="badge ${scoreType}">${scoreType.toUpperCase()}</span></div>
      <div class="lead-av" style="background:${avColor};">${av}</div>
      <div class="lead-main-info">
        <div class="lead-name">${lead.name||'—'} <span style="font-size:9px;background:#F3F4F6;color:var(--muted);padding:2px 6px;border-radius:4px;font-weight:600;">ARCHIVED</span></div>
        <div class="lead-sub">${lead.phone||'—'}</div>
        <div class="lead-sub">${intentShort} · ${lead.property_type||'—'}</div>
        <div class="lead-sub">${lead.area||'—'}</div>
      </div>
      ...
      <button onclick="unarchiveLead('${lead.id}')" style="background:rgba(16,185,129,0.1);color:#10B981;border:1px solid rgba(16,185,129,0.2);padding:5px 10px;border-radius:7px;font-size:11px;font-weight:600;cursor:pointer;font-family:'Inter',sans-serif;flex-shrink:0;">Restore</button>
    </div>`;
  }).join('');
  wrap.style.display = 'none';
}
...
function unarchiveLead(leadId) {
  ...
  // Restore lead to allLeads
  const lead = leadStore[leadId];
  if(lead && !allLeads.find(l=>l.id===leadId)) {
    allLeads.unshift(lead);
    filteredLeads = [...allLeads];
    filteredLeads2 = [...allLeads];
    updateStats();
  }
  showToast('Lead restore ho gaya!');
  renderArchivedLeads();
}
...
function saveTags(leadId){
  localStorage.setItem('eb_tags',JSON.stringify(leadTags));
}
...
function renderTagsSection(leadId){
  ...
  return `<div class="detail-section">
    ...
    <div style="display:flex;flex-wrap:wrap;gap:5px;margin-bottom:8px;" id="tagDisplay_${leadId}">
      ${currentTags.length ? currentTags.map(t=>`<span style="background:#EDE9FE;color:#5B3DF5;padding:3px 9px;border-radius:100px;font-size:11px;font-weight:600;cursor:pointer;" onclick="removeTag('${leadId}','${t}')">${t} ×</span>`).join('') : '<span style="font-size:12px;color:var(--muted);">No tags yet</span>'}
    </div>
    <select onchange="addTag('${leadId}',this.value);this.value='';" style="width:100%;border:1.5px solid var(--border);border-radius:8px;padding:7px 10px;font-size:12px;font-family:'Inter',sans-serif;outline:none;color:var(--text);">
      <option value="">+ Add Tag</option>
      ${TAG_OPTIONS.map(t=>`<option value="${t}">${t}</option>`).join('')}
    </select>
  </div>`;
}

function addTag(leadId, tag){
  if(!tag) return;
  if(!leadTags[leadId]) leadTags[leadId]=[];
  if(!leadTags[leadId].includes(tag)) leadTags[leadId].push(tag);
  saveTags(leadId);
  const el = document.getElementById('tagDisplay_'+leadId);
  if(el) el.innerHTML = leadTags[leadId].map(t=>`<span style="background:#EDE9FE;color:#5B3DF5;padding:3px 9px;border-radius:100px;font-size:11px;font-weight:600;cursor:pointer;" onclick="removeTag('${leadId}','${t}')">${t} ×</span>`).join('');
  showToast('Tag added!');
}

function removeTag(leadId, tag){
  if(!leadTags[leadId]) return;
  leadTags[leadId]=leadTags[leadId].filter(t=>t!==tag);
  saveTags(leadId);
  const el = document.getElementById('tagDisplay_'+leadId);
  if(el) el.innerHTML = leadTags[leadId].length ? leadTags[leadId].map(t=>`<span style="background:#EDE9FE;color:#5B3DF5;padding:3px 9px;border-radius:100px;font-size:11px;font-weight:600;cursor:pointer;" onclick="removeTag('${leadId}','${t}')">${t} ×</span>`).join('') : '<span style="font-size:12px;color:var(--muted);">No tags yet</span>';
}
...
function toggleBulkMode(){
  ...
  if(isBulk){
    ...
    document.querySelectorAll('.lead-checkbox').forEach(c=>c.style.display='none');
  } else {
    document.getElementById('bulkBar').style.display='flex';
    document.querySelectorAll('.lead-checkbox').forEach(c=>c.style.display='flex');
  }
}
...
async function bulkUpdateStatus(status){
  ...
  allLeads = allLeads.map(l=>selectedLeads.has(l.id)?{...l,status}:l);
  filteredLeads = filteredLeads.map(l=>selectedLeads.has(l.id)?{...l,status}:l);
  filteredLeads2 = filteredLeads2.map(l=>selectedLeads.has(l.id)?{...l,status}:l);
  updateStats();renderTable();
  showToast(`${selectedLeads.size} leads updated to ${status}! ✅`);
  selectedLeads.clear();
  document.getElementById('bulkBar').style.display='none';
}
...
function showMonthlyReport(){
  ...
  // Best day
  const dayCounts={};
  monthLeads.forEach(l=>{const d=new Date(l.created_at).toLocaleDateString('en-IN',{weekday:'long'});dayCounts[d]=(dayCounts[d]||0)+1;});
  ...
  document.getElementById('reportContent').innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
      <div style="background:#F8FAFC;border-radius:10px;padding:14px;text-align:center;">
        <div style="font-size:28px;font-weight:800;color:var(--navy);">${monthLeads.length}</div>
        <div style="font-size:11px;color:var(--muted);margin-top:3px;">Total Leads This Month</div>
      </div>
      <div style="background:#F8FAFC;border-radius:10px;padding:14px;text-align:center;">
        <div style="font-size:28px;font-weight:800;color:#10B981;">${converted}</div>
        <div style="font-size:11px;color:var(--muted);margin-top:3px;">Converted</div>
      </div>
      <div style="background:#F8FAFC;border-radius:10px;padding:14px;text-align:center;">
        <div style="font-size:28px;font-weight:800;color:#EF4444;">${hot}</div>
        <div style="font-size:11px;color:var(--muted);margin-top:3px;">Hot Leads</div>
      </div>
      <div style="background:#F8FAFC;border-radius:10px;padding:14px;text-align:center;">
        <div style="font-size:28px;font-weight:800;color:var(--gold);">${monthLeads.length?Math.round((converted/monthLeads.length)*100):0}%</div>
        <div style="font-size:11px;color:var(--muted);margin-top:3px;">Conversion Rate</div>
      </div>
    </div>
    <div style="background:#F8FAFC;border-radius:10px;padding:14px;margin-bottom:12px;">
      <div style="font-size:12px;font-weight:700;margin-bottom:8px;">📅 ${now.toLocaleString('en-IN',{month:'long',year:'numeric'})} Summary</div>
      <div style="font-size:13px;color:var(--text2);line-height:1.8;">
        ${bestDay?`🏆 Best day: <b>${bestDay[0]}</b> (${bestDay[1]} leads)<br>`:''}
        💰 Est. Commission: <b>₹${revEst.toLocaleString('en-IN')}</b> (@₹15k/deal)<br>
        📊 Avg per day: <b>${(monthLeads.length/now.getDate()).toFixed(1)} leads</b>
      </div>
    </div>
    <button onclick="exportMonthlyReport()" style="width:100%;background:var(--navy);color:#fff;border:none;padding:10px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;font-family:'Inter',sans-serif;">📥 Download Report</button>`;
...
function getBudgetValue(budget){
  if(!budget) return 0;
  const n=parseFloat(budget.replace(/[^0-9.]/g,''))||0;
...
document.addEventListener('keydown',e=>{
  ...
  if(e.key==='1') showPage('dashboard');
    ...
    if(currentBroker){loadLeads(currentBroker.broker_id);showToast('Refreshed! 🔄');}
  }
});

// ===== DAILY SUMMARY POPUP =====
function checkDailySummary(){
  const lastShown = localStorage.getItem('eb_summary_date');
  const today = new Date().toDateString();
  ...
  document.getElementById('summaryContent').innerHTML=`
    ...
    <div style="font-size:18px;font-weight:800;color:var(--navy);margin-bottom:6px;">Good morning, ${currentBroker?.name?.split(' ')[0]||''}!</div>
    <div style="font-size:13px;color:var(--muted);margin-bottom:20px;">Aaj ka summary</div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:18px;">
      <div style="background:#FEF3C7;border-radius:10px;padding:14px;text-align:center;">
        <div style="font-size:24px;font-weight:800;color:#D97706;">${todayFUs.length}</div>
        <div style="font-size:11px;color:#92400E;margin-top:3px;">Follow Ups Today</div>
      </div>
      <div style="background:#EDE9FE;border-radius:10px;padding:14px;text-align:center;">
        <div style="font-size:24px;font-weight:800;color:#5B3DF5;">${todayLeads.length}</div>
        <div style="font-size:11px;color:#3730A3;margin-top:3px;">New Leads Today</div>
      </div>
    </div>
    ${todayFUs.length>0?`<div style="background:#FEE2E2;border-radius:8px;padding:10px 12px;margin-bottom:12px;font-size:12px;color:#DC2626;font-weight:600;">⚠️ ${todayFUs.length} follow up${todayFUs.length>1?'s':''} pending!</div>`:''}
    <button onclick="document.getElementById('summaryModal').classList.remove('show');showPage('followups');" style="width:100%;background:var(--navy);color:#fff;border:none;padding:10px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;font-family:'Inter',sans-serif;margin-bottom:8px;">View Follow Ups</button>
    <button onclick="document.getElementById('summaryModal').classList.remove('show');" style="width:100%;background:transparent;color:var(--muted);border:1px solid var(--border);padding:10px;border-radius:8px;font-size:13px;cursor:pointer;font-family:'Inter',sans-serif;">Dismiss</button>`;
...
function checkAutoFollowUp(){
  ...
  const needsFollowUp = allLeads.filter(l=>{
    if(existingFULeadIds.has(l.id)) return false;
    if(l.status==='converted'||l.status==='not_interested') return false;
    if(!l.created_at) return false;
    const logsForLead = callLogs[l.id]||[];
    const daysSince = (Date.now()-new Date(l.created_at))/(1000*60*60*24);
    return daysSince>=3 && logsForLead.length===0;
  });
  if(needsFollowUp.length>0){
    const tomorrow = new Date();tomorrow.setDate(tomorrow.getDate()+1);
    const tomorrowStr = tomorrow.toISOString().split('T')[0];
    let added=0;
    needsFollowUp.slice(0,5).forEach(l=>{
      if(!followUps.find(f=>f.leadId===l.id&&!f.done)){
        followUps.push({id:Date.now().toString()+Math.random(),brokerId,leadId:l.id,leadName:l.name||'Lead',date:tomorrowStr,note:'Auto: 3 din se call nahi hua',done:false});
        added++;
      }
    });
    if(added>0){
      localStorage.setItem('eb_followups',JSON.stringify(followUps));
      showToast(`${added} auto follow up set ho gaye!`);
    }
  }
}
...
function saveProperty(){
  const title=document.getElementById('propTitle').value.trim(),type=document.getElementById('propType').value,location=document.getElementById('propLocation').value.trim(),price=document.getElementById('propPrice').value.trim(),status=document.getElementById('propStatus').value,details=document.getElementById('propDetails').value.trim();
  if(!title||!location||!price){showToast('Title, location aur price zaroori hai','error');return;}
  properties.push({id:Date.now().toString(),title,type,location,price,status,details,addedOn:new Date().toISOString()});
  localStorage.setItem('eb_properties',JSON.stringify(properties));
  document.getElementById('propModal').classList.remove('show');
  ['propTitle','propLocation','propPrice','propDetails'].forEach(id=>document.getElementById(id).value='');
  renderProperties();showToast('Property add ho gayi!');
}
function renderProperties(){
  ...
  document.getElementById('propStats').innerHTML=`
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;"><div style="font-size:20px;font-weight:800;color:var(--navy);">${properties.length}</div><div style="font-size:11px;color:var(--muted);">Total</div></div>
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;"><div style="font-size:20px;font-weight:800;color:#10B981;">${properties.filter(p=>p.status==='available').length}</div><div style="font-size:11px;color:var(--muted);">Available</div></div>
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;"><div style="font-size:20px;font-weight:800;color:#5B3DF5;">${properties.filter(p=>p.status==='sold').length}</div><div style="font-size:11px;color:var(--muted);">Sold</div></div>`;
  ...
  document.getElementById('propList').innerHTML=filtered.map(p=>`
    <div style="display:flex;align-items:center;gap:12px;padding:12px 14px;border-bottom:1px solid var(--border);">
      <div style="width:36px;height:36px;border-radius:9px;background:#EDE9FE;display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0;">${p.type==='flat'?'🏢':p.type==='house'?'🏠':p.type==='plot'?'🌍':'🏪'}</div>
      <div style="flex:1;min-width:0;">
        <div style="font-size:13px;font-weight:600;">${p.title}</div>
        <div style="font-size:11px;color:var(--muted);">📍 ${p.location} · ${p.type}${p.details?' · '+p.details:''}</div>
      </div>
      <div style="text-align:right;flex-shrink:0;">
        <div style="font-size:13px;font-weight:700;">₹${p.price}</div>
        <span style="background:${sb[p.status]};color:${sc[p.status]};padding:2px 7px;border-radius:4px;font-size:9px;font-weight:700;">${p.status.toUpperCase()}</span>
      </div>
      <select onchange="updatePropStatus('${p.id}',this.value)" style="border:1px solid var(--border);border-radius:7px;padding:5px 7px;font-size:11px;font-family:'Inter',sans-serif;outline:none;">
      ...
      <button onclick="if(confirm('Delete?')){properties=properties.filter(x=>x.id!=='${p.id}');localStorage.setItem('eb_properties',JSON.stringify(properties));renderProperties();showToast('Deleted!')}" style="background:none;border:none;color:#EF4444;cursor:pointer;font-size:14px;">🗑</button>
    </div>`).join('');
}
...
function openAddVisit(){
  ...
  document.getElementById('visitLeadSelect').innerHTML='<option value="">-- Lead Select Karo --</option>'+allLeads.map(l=>`<option value="${l.id}|${l.name}|${l.phone||''}">${l.name} — ${l.phone||''}</option>`).join('');
}
function saveVisit(){
  const lv=document.getElementById('visitLeadSelect').value,property=document.getElementById('visitProperty').value.trim(),date=document.getElementById('visitDate').value,time=document.getElementById('visitTime').value,notes=document.getElementById('visitNotes').value.trim();
  if(!lv||!property||!date){showToast('Lead, property aur date zaroori hai','error');return;}
  const[leadId,leadName,leadPhone]=lv.split('|');
  siteVisits.push({id:Date.now().toString(),leadId,leadName,leadPhone,property,date,time,notes,status:'scheduled',feedback:''});
  localStorage.setItem('eb_visits',JSON.stringify(siteVisits));
  document.getElementById('visitModal').classList.remove('show');
  renderVisits();showToast('Site visit schedule ho gayi!');
}
function renderVisits(){
  ...
  document.getElementById('visitStats').innerHTML=`
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;"><div style="font-size:20px;font-weight:800;color:var(--navy);">${siteVisits.length}</div><div style="font-size:11px;color:var(--muted);">Total</div></div>
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;"><div style="font-size:20px;font-weight:800;color:var(--gold);">${siteVisits.filter(v=>new Date(v.date+'T00:00:00').toDateString()===today).length}</div><div style="font-size:11px;color:var(--muted);">Aaj</div></div>
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;"><div style="font-size:20px;font-weight:800;color:#5B3DF5;">${siteVisits.filter(v=>v.status==='scheduled').length}</div><div style="font-size:11px;color:var(--muted);">Scheduled</div></div>
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;"><div style="font-size:20px;font-weight:800;color:#10B981;">${siteVisits.filter(v=>v.status==='completed').length}</div><div style="font-size:11px;color:var(--muted);">Completed</div></div>`;
  ...
  document.getElementById('visitList').innerHTML=filtered.sort((a,b)=>a.date.localeCompare(b.date)).map(v=>`
    <div style="padding:12px 14px;border-bottom:1px solid var(--border);">
      <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:8px;">
        <div style="flex:1;">
          <div style="font-size:13px;font-weight:600;">${v.leadName} <span style="background:${sb[v.status]};color:${sc[v.status]};padding:2px 7px;border-radius:4px;font-size:9px;font-weight:700;">${v.status.toUpperCase()}</span></div>
          <div style="font-size:11px;color:var(--muted);">📞 ${v.leadPhone} · 🏠 ${v.property}</div>
          <div style="font-size:11px;color:var(--muted);">📅 ${new Date(v.date+'T00:00:00').toLocaleDateString('en-IN',{day:'numeric',month:'short'})} ${v.time?'⏰ '+v.time:''}</div>
          ${v.feedback?`<div style="font-size:11px;color:#10B981;">💬 ${v.feedback}</div>`:''}
        </div>
        <button onclick="if(confirm('Delete?')){siteVisits=siteVisits.filter(x=>x.id!=='${v.id}');localStorage.setItem('eb_visits',JSON.stringify(siteVisits));renderVisits();}" style="background:none;border:none;color:#EF4444;cursor:pointer;font-size:13px;">🗑</button>
      </div>
      ${v.status==='scheduled'?`<div style="display:flex;gap:6px;">
        <button onclick="siteVisits=siteVisits.map(x=>x.id==='${v.id}'?{...x,status:'completed'}:x);localStorage.setItem('eb_visits',JSON.stringify(siteVisits));renderVisits();showToast('✅ Completed!')" style="background:#D1FAE5;color:#065F46;border:none;padding:5px 10px;border-radius:7px;font-size:11px;font-weight:600;cursor:pointer;font-family:'Inter',sans-serif;">✓ Done</button>
        <button onclick="const fb=prompt('Feedback:');if(fb){siteVisits=siteVisits.map(x=>x.id==='${v.id}'?{...x,feedback:fb}:x);localStorage.setItem('eb_visits',JSON.stringify(siteVisits));renderVisits();}" style="background:#EDE9FE;color:#5B3DF5;border:none;padding:5px 10px;border-radius:7px;font-size:11px;font-weight:600;cursor:pointer;font-family:'Inter',sans-serif;">💬 Feedback</button>
        <button onclick="siteVisits=siteVisits.map(x=>x.id==='${v.id}'?{...x,status:'cancelled'}:x);localStorage.setItem('eb_visits',JSON.stringify(siteVisits));renderVisits();" style="background:#FEE2E2;color:#DC2626;border:none;padding:5px 10px;border-radius:7px;font-size:11px;font-weight:600;cursor:pointer;font-family:'Inter',sans-serif;">✕ Cancel</button>
      </div>`:''}
    </div>`).join('');
}
...
function addDeal(){
  const client=document.getElementById('dealClient').value.trim(),property=document.getElementById('dealProperty').value.trim(),amount=document.getElementById('dealAmount').value.trim(),commPct=parseFloat(document.getElementById('dealCommission').value)||2,date=document.getElementById('dealDate').value||new Date().toISOString().split('T')[0];
  if(!client||!property||!amount){showToast('Client, property aur amount zaroori hai','error');return;}
  const n=parseFloat(amount.replace(/[^0-9.]/g,''))||0;
  const amtRs=amount.toLowerCase().includes('lakh')||amount.toLowerCase().includes('lac')?n*100000:amount.toLowerCase().includes('cr')?n*10000000:n;
  const comm=Math.round(amtRs*(commPct/100));
  deals.push({id:Date.now().toString(),client,property,amount,amtRs,commPct,commission:comm,date});
  localStorage.setItem('eb_deals',JSON.stringify(deals));
  ['dealClient','dealProperty','dealAmount','dealCommission'].forEach(id=>document.getElementById(id).value='');
  renderRevenue();showToast('Deal save ho gayi! 💰');
}
...
function renderRevenue(){
  ...
  const md=deals.filter(d=>new Date(d.date)>=ms);
  const tc=deals.reduce((s,d)=>s+(d.commission||0),0),mc=md.reduce((s,d)=>s+(d.commission||0),0),target=parseInt(localStorage.getItem('eb_target'))||0;
  document.getElementById('revenueStats').innerHTML=`
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;"><div style="font-size:20px;font-weight:800;color:var(--navy);">${deals.length}</div><div style="font-size:11px;color:var(--muted);">Total Deals</div></div>
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;"><div style="font-size:20px;font-weight:800;color:#10B981;">₹${(tc/100000).toFixed(1)}L</div><div style="font-size:11px;color:var(--muted);">Total Earnings</div></div>
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;"><div style="font-size:20px;font-weight:800;color:var(--gold);">₹${(mc/100000).toFixed(1)}L</div><div style="font-size:11px;color:var(--muted);">This Month</div></div>
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;"><div style="font-size:20px;font-weight:800;color:#5B3DF5;">${md.length}</div><div style="font-size:11px;color:var(--muted);">Deals This Month</div></div>`;
  if(target>0){const pct=Math.min((mc/target)*100,100);document.getElementById('targetProgress').innerHTML=`<div style="font-size:12px;color:var(--muted);margin-bottom:6px;">${mc.toLocaleString('en-IN')} / ₹${target.toLocaleString('en-IN')}</div><div style="height:8px;background:var(--border);border-radius:100px;overflow:hidden;"><div style="height:100%;width:${pct}%;background:${pct>=100?'#10B981':'#5B3DF5'};border-radius:100px;"></div></div><div style="font-size:12px;font-weight:700;color:${pct>=100?'#10B981':'#5B3DF5'};margin-top:6px;">${pct.toFixed(0)}% of target</div>`;}
  document.getElementById('dealsList').innerHTML=deals.length?deals.slice().reverse().map(d=>`<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border);"><div style="flex:1;"><div style="font-size:13px;font-weight:600;">${d.client}</div><div style="font-size:11px;color:var(--muted);">${d.property} · ${new Date(d.date).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'})}</div></div><div style="text-align:right;"><div style="font-size:13px;font-weight:700;color:#10B981;">+₹${d.commission.toLocaleString('en-IN')}</div><div style="font-size:10px;color:var(--muted);">${d.commPct}% of ${d.amount}</div></div><button onclick="deals=deals.filter(x=>x.id!=='${d.id}');localStorage.setItem('eb_deals',JSON.stringify(deals));renderRevenue();" style="background:none;border:none;color:#EF4444;cursor:pointer;font-size:13px;">🗑</button></div>`).join(''):'<div style="color:var(--muted);text-align:center;padding:20px;font-size:13px;">Koi deal nahi — Add karo!</div>';
}

// ===== DOCUMENTS =====
const DOCS={
  buyer:['✅ Aadhaar Card (buyer)','✅ PAN Card','✅ Passport size photos (4)','✅ Bank statements (6 months)','✅ Salary slips / ITR (last 2 years)','✅ Sale deed / Agreement','✅ NOC from society','✅ Loan sanction letter (if applicable)','✅ Cheque for registration fees'],
  seller:['✅ Original title deed / Registry','✅ Aadhaar Card (seller)','✅ PAN Card','✅ Encumbrance certificate','✅ Property tax receipts (last 3 years)','✅ NOC from society / builder','✅ Approved building plan','✅ Possession letter','✅ Bank NOC (if loan on property)','✅ Capital gains tax document'],
  rental:['✅ Aadhaar Card (tenant)','✅ PAN Card (tenant)','✅ 2 Passport photos','✅ Employment / salary proof','✅ Rent agreement (stamp paper)','✅ Police verification form','✅ Security deposit cheque','✅ Aadhaar of owner','✅ Property ownership proof'],
  plot:['✅ Sale deed / Registry','✅ Khasra / Khatauni copy','✅ Map / Layout plan','✅ NOC from municipality','✅ Mutation certificate','✅ Land use certificate','✅ Encumbrance certificate','✅ Aadhaar + PAN (both parties)','✅ Jamabandi (last 15 years)']
};
function renderDocuments(){
  ['buyer','seller','rental','plot'].forEach(t=>{document.getElementById(t+'Docs').innerHTML=DOCS[t].map(d=>`<div style="font-size:12px;padding:6px 0;border-bottom:1px solid #F1F5F9;">${d}</div>`).join('');});
}
function copyChecklist(type){navigator.clipboard.writeText(DOCS[type].join('\n'));showToast('Checklist copy ho gaya! 📋');}

// ===== CALCULATORS =====
function calcEMI(){
  const loan=parseFloat(document.getElementById('emiLoan').value)||0,rate=parseFloat(document.getElementById('emiRate').value)||0,years=parseFloat(document.getElementById('emiTenure').value)||0;
  if(!loan||!rate||!years)return;
  const r=rate/(12*100),n=years*12,emi=Math.round(loan*(r*Math.pow(1+r,n))/(Math.pow(1+r,n)-1)),totalPay=emi*n,totalInt=totalPay-loan;
  document.getElementById('emiResult').style.display='block';
  document.getElementById('emiAmount').textContent='₹'+emi.toLocaleString('en-IN')+'/month';
  document.getElementById('emiBreakdown').innerHTML=`Loan: ₹${loan.toLocaleString('en-IN')}<br>Total Payment: ₹${totalPay.toLocaleString('en-IN')}<br>Total Interest: ₹${totalInt.toLocaleString('en-IN')}`;
}
function setCommRate(rate){document.getElementById('commRate').value=rate;calcCommission();}
function calcCommission(){
  const val=parseFloat(document.getElementById('commValue').value)||0,rate=parseFloat(document.getElementById('commRate').value)||0;
  if(!val||!rate)return;
  const comm=Math.round(val*(rate/100)),gst=Math.round(comm*0.18);
  document.getElementById('commResult').style.display='block';
  document.getElementById('commAmount').textContent='₹'+comm.toLocaleString('en-IN');
  document.getElementById('commBreakdown').innerHTML=`Property: ₹${val.toLocaleString('en-IN')}<br>Commission @${rate}%: ₹${comm.toLocaleString('en-IN')}<br>GST @18%: ₹${gst.toLocaleString('en-IN')}<br>Net: ₹${(comm-gst).toLocaleString('en-IN')}`;
}
function calcStamp(){
  const val=parseFloat(document.getElementById('stampValue').value)||0,state=document.getElementById('stampState').value;
  if(!val)return;
  const rates={RJ:6,MH:5,DL:5,UP:7,HR:6,KA:5,PB:6,GJ:4.9},r=rates[state]||5;
  const stamp=Math.round(val*(r/100)),reg=Math.round(val*0.01);
  document.getElementById('stampResult').style.display='block';
  document.getElementById('stampBreakdown').innerHTML=`Property: ₹${val.toLocaleString('en-IN')}<br>Stamp Duty @${r}%: ₹${stamp.toLocaleString('en-IN')}<br>Registration @1%: ₹${reg.toLocaleString('en-IN')}<br><b>Total: ₹${(stamp+reg).toLocaleString('en-IN')}</b>`;
}
function calcRentYield(){
  const pv=parseFloat(document.getElementById('rentPropVal').value)||0,rm=parseFloat(document.getElementById('rentMonthly').value)||0;
  if(!pv||!rm)return;
  const annual=rm*12,yld=((annual/pv)*100).toFixed(2),payback=Math.round(pv/annual);
  document.getElementById('rentYieldResult').style.display='block';
  document.getElementById('rentYieldBreakdown').innerHTML=`Annual Rent: ₹${annual.toLocaleString('en-IN')}<br>Rental Yield: <b>${yld}%/year</b><br>Payback: ~${payback} years<br>${parseFloat(yld)>=3?'✅ Achha return!':'⚠️ Return kam hai'}`;
}

// ===== OVERRIDE SHOWPAGE for new pages =====
showPage = function(page){
  ...
  document.querySelectorAll('.nav-item').forEach(n=>{if(n.getAttribute('onclick')&&n.getAttribute('onclick').includes("'"+page+"'"))n.classList.add('active');});
  const titles={dashboard:'Dashboard',leads:'All Leads',analytics:'Analytics',followups:'Follow Ups',settings:'Settings',properties:'Property Inventory',visits:'Site Visits',revenue:'Revenue',documents:'Documents',calculator:'Calculators'};
  document.getElementById('pageTitle').textContent=titles[page]||page;
  if(page==='leads')renderLeadsPage();
  if(page==='analytics')renderAnalytics();
  if(page==='followups')renderFollowUps();
