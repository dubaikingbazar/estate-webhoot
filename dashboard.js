...
function logCall(leadId,leadName){const now=new Date().toLocaleString('en-IN',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'});const result=prompt('Call result likho');if(result===null)return;if(!callLogs[leadId])callLogs[leadId]=[];callLogs[leadId].push(`📞 ${now} — ${result||'Called'}`);localStorage.setItem('eb_calllogs',JSON.stringify(callLogs));const el=document.getElementById('callLogDisplay_'+leadId);if(el)el.innerHTML=callLogs[leadId].slice(-3).map(l=>`<div style="font-size:11px;color:var(--muted);padding:4px 0;border-bottom:1px solid rgba(232,224,208,0.05);">${l}</div>`).join('');showToast('Call logged!');}
...
function showSubBanner(broker) {
  ...
  if (broker.status === 'trial' && broker.trial_start) {
    ...
    daysEl.textContent = 'Plan khatam ho gaya';
    btn.style.display = 'block';
